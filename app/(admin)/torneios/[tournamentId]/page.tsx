...
export default async function TournamentDetailPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  const [{ data: allOrganizers }, { data: linkedOrganizers }, { data: allAthletes }, { data: linkedAthletes }, { data: categories }, { data: teams }] = await Promise.all([
    ...
    <main className="min-h-screen px-6 py-10 max-w-2xl mx-auto">
      ...
      <div className="mt-10 pt-10 border-t border-white/10">
        <TournamentTeams
          ...
          categories={categories ?? []}
          linkedAthletes={linkedAthletesWithCategory}
          teams={(teams ?? []).map((t: any) => ({ id: t.id, category_id: t.category_id, athlete_1: t.athlete_1, athlete_2: t.athlete_2 }))}
        />
      </div>
    </main>
  )
